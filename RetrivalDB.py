...
wiki_wiki = wikipediaapi.Wikipedia('Psychology_LLM_Agent (fan.qih@northeastern.edu)', 'en')
...
def textEmbed(text):
    try:
        ...
        response = openai.Embedding.create(
            input=[text],  # Wrap input in a list
            model=model
        )
...
def main():
    ...
    if not os.path.isfile(csv_file):
        ...
        for idx, topic in tqdm(enumerate(all_topics), total=len(all_topics)):
            ...
            data_list.append({'_id': idx, 'Topic': topic, 'Summary': summary, 'Embeded': embeded})
            time.sleep(1)
    ...
    if 'Embeded' in data.columns and not data['Embeded'].isnull().all():
        ...
        data['Embeded'] = data['Embeded'].apply(lambda x: np.fromstring(x.strip("[]"), sep=',') if isinstance(x, str) else x)
        ...
        for doc in collection.find({}, {'_id': 1, 'Embeded': 1}):
            try:
                if 'Embeded' not in doc or not doc['Embeded']:
                    print(f"Skipping document {doc['_id']}: No 'Embeded' field or empty embedding.")
                    continue

                embedding = np.array(doc['Embeded'], dtype='float32')
                if embedding.shape != (dimension,):
                    print(f"Skipping document {doc['_id']}: Incorrect shape {embedding.shape}.")
                    continue
