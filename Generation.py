...
def diagnostic_agent(formatted_documents, initial_inputs, formatted_followup, model):
    try:
        ...
        if likelihood_match:
            likelihoods = [int(score.strip()) for score in likelihood_match.group(1).split(",")]
...
def vote_for_results(documents, initial_inputs, followupQ, followupA):
    ...
    formatted_documents = "\n".join([f"- {topic}: {summary}" for topic, summary in documents.items()])
    formatted_followup = "\n".join([f"Q: {q}\nA: {a}" for q, a in zip(followupQ, followupA)])
    
    # Store diagnostics from each agent
    diagnostics = {}
    for model in judges:
        diagnoses, likelihoods = diagnostic_agent(formatted_documents, initial_inputs, formatted_followup, model)
        normalized_diagnoses = [normalize_diagnosis(diag) for diag in diagnoses]
        diagnostics[model] = {diag: likelihood for diag, likelihood in zip(normalized_diagnoses, likelihoods)}

    # Collect all unique diagnoses
    all_diagnoses = set(diag for result in diagnostics.values() for diag in result.keys())

    # Create a DataFrame with rows as diagnoses and columns as models
    table_data = {model: {diag: diagnostics[model].get(diag, 0) for diag in all_diagnoses} for model in judges}
    ...
    for model in judges:
        ...
        normalized_second_diagnoses = [normalize_diagnosis(diag) for diag in second_round_diagnoses]
        second_round_diagnostics[model] = {
            diag: likelihood for diag, likelihood in zip(normalized_second_diagnoses, second_round_likelihoods)
        }

    # Combine second round results into a DataFrame
    second_round_table_data = {
        model: {diag: second_round_diagnostics[model].get(diag, 0) for diag in shortlisted_diagnoses} for model in judges
    }
...
def final_agent(selected_agent, selected_diagnosis, documents, initial_inputs, followupQ, followupA):

    # Format documents and follow-up into strings for context
    formatted_documents = "\n".join([f"- {topic}: {summary}" for topic, summary in documents.items()])
    formatted_followup = f"Follow-Up Questions:\n{followupQ}\n\nFollow-Up Answers:\n{followupA}"
